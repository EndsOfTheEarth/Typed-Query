...
namespace Sql.Column {
	...
	public class NDecimalColumn : AColumn {

		public NDecimalColumn(ATable pTable, string pColumnName)
			: base(pTable, pColumnName, false, true) {
		}
		public NDecimalColumn(ATable pTable, string pColumnName, bool pIsPrimaryKey)
			: base(pTable, pColumnName, pIsPrimaryKey, true) {
		}

		public static Condition operator ==(NDecimalColumn pColumnA, NDecimalColumn pColumnB) {
			return new ColumnCondition(pColumnA, Sql.Operator.EQUALS, pColumnB);
		}
		public static Condition operator ==(NDecimalColumn pColumnA, DecimalColumn pColumnB) {
			return new ColumnCondition(pColumnA, Sql.Operator.EQUALS, pColumnB);
		}
		public static Condition operator ==(NDecimalColumn pColumnA, decimal pValue) {
		...
		public static NumericCondition<DecimalColumn, NDecimalColumn, decimal> operator +(NDecimalColumn pColumnA, NDecimalColumn pColumnB) {
			return new NumericCondition<DecimalColumn, NDecimalColumn, decimal>(pColumnA, NumericOperator.ADD, pColumnB);
		}
		public static NumericCondition<DecimalColumn, NDecimalColumn, decimal> operator +(NDecimalColumn pColumnA, DecimalColumn pColumnB) {
			return new NumericCondition<DecimalColumn, NDecimalColumn, decimal>(pColumnA, NumericOperator.ADD, pColumnB);
		}
		public static NumericCondition<DecimalColumn, NDecimalColumn, decimal> operator +(NDecimalColumn pColumnA, decimal pValue) {
		...
		public override object GetValue(ADatabase pDatabase, System.Data.Common.DbDataReader pReader, int pColumnIndex) {
			...
			if(dataType != typeof(decimal) && dataType != typeof(decimal?))
				throw new Exception("Row column data is not of the correct type. Expected decimal or decimal? value instead got '" + dataType.ToString() + "'. This probably means that the database and table column data types are not matching. Please run the definition tester to check table columns are of the correct type. Table: '" + Table.TableName + "' Column: '" + ColumnName + "'");
