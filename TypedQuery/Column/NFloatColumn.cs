...
namespace Sql.Column {
    ...
    public class NFloatColumn : AColumn {

        public NFloatColumn(ATable pTable, string pColumnName)
            : base(pTable, pColumnName, false, true) {
        }
        public NFloatColumn(ATable pTable, string pColumnName, bool pIsPrimaryKey)
            : base(pTable, pColumnName, pIsPrimaryKey, true) {
        }

        public static Condition operator ==(NFloatColumn pColumnA, NFloatColumn pColumnB) {
            return new ColumnCondition(pColumnA, Sql.Operator.EQUALS, pColumnB);
        }
        public static Condition operator ==(NFloatColumn pColumnA, FloatColumn pColumnB) {
            return new ColumnCondition(pColumnA, Sql.Operator.EQUALS, pColumnB);
        }
        public static Condition operator ==(NFloatColumn pColumnA, float pValue) {
        ...
        public static NumericCondition<FloatColumn, NFloatColumn, float> operator +(NFloatColumn pColumnA, NFloatColumn pColumnB) {
            return new NumericCondition<FloatColumn, NFloatColumn, float>(pColumnA, NumericOperator.ADD, pColumnB);
        }
        public static NumericCondition<FloatColumn, NFloatColumn, float> operator +(NFloatColumn pColumnA, FloatColumn pColumnB) {
            return new NumericCondition<FloatColumn, NFloatColumn, float>(pColumnA, NumericOperator.ADD, pColumnB);
        }
        public static NumericCondition<FloatColumn, NFloatColumn, float> operator +(NFloatColumn pColumnA, float pValue) {
        ...
        public override object? GetValue(ADatabase pDatabase, System.Data.Common.DbDataReader pReader, int pColumnIndex) {
            ...
            if(dataType != typeof(float) && dataType != typeof(float?)) {
                throw new Exception($"Row column data is not of the correct type. Expected float or float? value instead got '{ dataType.ToString() }'. This probably means that the database and table column data types are not matching. Please run the definition tester to check table columns are of the correct type. Table: '{ Table.TableName }' Column: '{ ColumnName }'");
            }
